// Generated by `xtask/gen-entities.py` from
// <https://html.spec.whatwg.org/entities.json> — do not edit by hand, regenerate instead.
//
// Named character reference table for the tokenizer's
// "Named character reference state" (WHATWG HTML spec §13.2.5).
// Each entry's name already includes a trailing `;` where the
// spec requires one; legacy (pre-HTML5) names appear twice, once
// with and once without `;`, both mapping to the same
// replacement text — matching upstream `entities.json` exactly.

pub(crate) static NAMED_CHARACTER_REFERENCES: [(&str, &str); 2231] = [
    ("AElig", "\u{c6}"),
    ("AElig;", "\u{c6}"),
    ("AMP", "\u{26}"),
    ("AMP;", "\u{26}"),
